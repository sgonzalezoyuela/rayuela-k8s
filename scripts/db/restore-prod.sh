#!/usr/bin/env bash
# =============================================================================
# restore-prod.sh - Restore a .pgdump backup into the K8s prod PostgreSQL pod
# =============================================================================
# Restores a pg_dump custom-format backup (from backup-prod.sh or the restic
# CronJob) into the Kubernetes prod PostgreSQL StatefulSet pod.
#
# This drops and recreates the 'rayuela' database, so ALL prod data is lost.
# The restored database will contain prod schemas (public + tenant_*) and data.
#
# Prerequisites:
#   - kubectl configured with access to the rayuela-prod namespace
#   - Pod 'rayuela-db-0' running in namespace 'rayuela-prod'
#
# Usage:
#   ./scripts/db/restore-prod.sh <file.pgdump>
#   ./scripts/db/restore-prod.sh --list
#
# Options:
...
echo ""
echo -e "${BOLD}=========================================="
echo -e "Rayuela prod (K8s) Database Restore"
echo -e "==========================================${NC}"
echo ""
...
echo -e "  ${RED}All existing  odata will be lost.${NC}"
echo ""
...
# ─────────────────────────────────────────────────────────────────
# 4. Grant permissions (match prod init-databases.sh)
# ─────────────────────────────────────────────────────────────────
echo ""
